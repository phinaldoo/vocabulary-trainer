...
export function SettingsPage() {
  ...
  const [sectionId, setSectionId] = useState(user?.selected_section_id ?? '');
  ...
  async function save(event: FormEvent) {
    ...
    try {
      const next = await api<User>('/account/settings', {
        method: 'PATCH',
        body: JSON.stringify({
          ...
          selected_section_id: sectionId || null,
        }),
      });
      updateUser(next); setDeckId(next.selected_deck_id ?? ''); setSaved(true);
      window.setTimeout(() => setSaved(false), 2500);
    } catch (caught) { setError(caught instanceof ApiError ? caught.message : t('settings.saveError')); }
  ...
  return <main className="page-wrap settings-page">
    ...
    <section className="settings-grid">
      <form className="panel settings-form" onSubmit={save}>
        ...
        <label><span>{t('common.language')}</span><div className="select-wrap"><select value={interfaceLanguage} onChange={(event) => { const next = event.target.value as UiLanguage; setInterfaceLanguage(next); setLanguage(next); }}>{supportedLanguages.map((value) => <option key={value} value={value}>{languageNames[value]}</option>)}</select><ChevronDown /></div></label>
        {selectedDeck && <><label><span>{t('settings.defaultDeck')}</span><div className="select-wrap"><select value={selectedDeck.id} onChange={(event) => { setDeckId(event.target.value); setSectionId(''); }}>{decks.data?.map((deck) => <option key={deck.id} value={deck.id}>{deck.title}</option>)}</select><ChevronDown /></div></label><label><span>{t('settings.defaultSection')}</span><div className="select-wrap"><select value={sectionId} onChange={(event) => setSectionId(event.target.value)}><option value="">{t('common.allSections')}</option>{sections.data?.map((section) => <option key={section.id} value={section.id}>{section.title}</option>)}</select><ChevronDown /></div></label></>}
        <label><span>{t('settings.dailyGoal')}</span><div className="select-wrap"><select value={dailyGoal} onChange={(event) => setDailyGoal(Number(event.target.value))}>{[5, 10, 12, 20, 30, 50].map((value) => <option key={value} value={value}>{t(value === 1 ? 'common.cardsOne' : 'common.cardsMany', { count: value })}</option>)}</select><ChevronDown /></div></label>
        <label><span>{t('settings.direction')}</span><div className="select-wrap"><select value={direction} onChange={(event) => setDirection(event.target.value as Direction)}><option value="forward">{selectedDeck ? `${selectedDeck.front_label} → ${selectedDeck.back_label}` : t('settings.frontToBack')}</option><option value="reverse">{selectedDeck ? `${selectedDeck.back_label} → ${selectedDeck.front_label}` : t('settings.backToFront')}</option><option value="mixed">{t('settings.mixed')}</option></select><ChevronDown /></div></label>
        <fieldset><legend>{t('settings.defaultMode')}</legend><div className="settings-modes"><label className={inputMode === 'typing' ? 'selected' : ''}><input type="radio" name="mode" checked={inputMode === 'typing'} onChange={() => setInputMode('typing')} /><strong>{t('settings.typeAnswer')}</strong><small>{t('settings.typeAnswerHint')}</small></label><label className={inputMode === 'reveal' ? 'selected' : ''}><input type="radio" name="mode" checked={inputMode === 'reveal'} onChange={() => setInputMode('reveal')} /><strong>{t('settings.revealCard')}</strong><small>{t('settings.revealCardHint')}</small></label></div></fieldset>
        {error && <p className="form-error" role="alert">{error}</p>}
        <button className="button primary" disabled={saving}>{saved ? <><Check /> {t('common.saved')}</> : saving ? t('common.saving') : <><Save /> {t('settings.saveChanges')}</>}</button>
      </form>
