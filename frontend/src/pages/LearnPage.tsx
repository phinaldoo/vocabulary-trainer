...
import { HighlightedSolution } from '../components/HighlightedSolution';
import type { Deck, Direction, InputMode, ReviewResult, Section, SelectionMode, StudySession } from '../types';
import { useI18n } from '../i18n';
import type { MessageKey } from '../i18n/messages';

type CheckedAnswer = { correct: boolean; solution: string; match_kind: string | null };
  ...
  return (
    <main className="study-shell">
      ...
      <section className="study-stage">
        ...
        {session.input_mode === 'typing' && checked && <section className={`answer-result ${checked.correct ? 'correct' : 'incorrect'}`}><header className="answer-result-header">{checked.correct ? <Check /> : <X />}<div className="answer-result-copy"><strong>{checked.correct ? t('landing.correct') : t('learn.notQuite')}</strong><span>{t('learn.solutionInline')} <HighlightedSolution solution={solution} answer={answer} language={card.answer_language} /></span></div></header><div className="rating-grid">{ratingOptions.map((option) => <button ref={option.value === 2 ? actionRef : undefined} type="button" key={option.value} className={option.tone} disabled={submitting} onClick={() => void rate(option.value)}>{option.value === 0 && <RotateCcw />}<span>{t(option.label as MessageKey)}<small>{t('learn.key', { key: option.key })}</small></span></button>)}</div></section>}
